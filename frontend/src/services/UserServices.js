...
export const addFavorite = async (id, productId) => {
    const { data } = await axios.post(`http://localhost:4000/users/${id}/favorite/${productId}`);
    return data;
};

export const deleteFavorite = async (id, productId) => {
    const { data } = await axios.delete(`http://localhost:4000/users/${id}/favorite/${productId}`);
    return data;
};